...
int main(int argc, char **argv)
{
    ...
    if (strcmp(cmd, "hash") == 0)
    {
        const char *usage = "Usage:\n"
            "  crypto hash sha1 <hex>\n"
            "  crypto hash sha256 <hex>\n"
            "  crypto hash sha3-256 <hex>\n"
            "  crypto hash ripemd160 <hex>\n";
    ...
    {
        const char *usage = "Usage:\n"
            "  crypto hmac sha1 <key-hex> <msg-hex>\n"
            "  crypto hmac sha256 <key-hex> <msg-hex>\n";
        if (argc < 5) { fprintf(stderr, "%s", usage); return 1; }
        const char *algo = argv[2];
    ...
    else if (strcmp(cmd, "pbkdf2") == 0)
    {
        const char *usage = "Usage:\n"
            "  crypto pbkdf2 <pass-hex> <salt-hex> <iter> <dklen> [sha1|sha256]\n";
        if (argc < 6) { fprintf(stderr, "%s", usage); return 1; }
    ...
    {
        const char *usage = "Usage:\n"
            "  crypto base64 enc <hex>\n"
            "  crypto base64 dec <string>\n";
        if (argc < 4) { fprintf(stderr, "%s", usage); return 1; }
    ...
    else if (strcmp(cmd, "utf8") == 0)
    {
        const char *usage = "Usage:\n"
            "  crypto utf8 enc <codes>\n"
            "  crypto utf8 dec <hex>\n";
        if (argc < 4) { fprintf(stderr, "%s", usage); return 1; }
        const char *op = argv[2];
        if (strcmp(op, "enc") == 0)
...
#define CMD_CIPHER_ECB(name, prefix, KEY_SIZE) \
    ...
    { \
        if (argc < 5) { fprintf(stderr, "Usage:\n" \
            "  crypto %s enc <key-hex> <data-hex>\n" \
            "  crypto %s dec <key-hex> <data-hex>\n", name, name); return 1; } \
        const char *op = argv[2]; \
...
#define CMD_CIPHER_CBC(name, prefix, KEY_SIZE, BLOCK_SIZE) \
    ...
    { \
        if (argc < 6) { fprintf(stderr, "Usage:\n" \
            "  crypto %s enc <key-hex> <iv-hex> <data-hex>\n" \
            "  crypto %s dec <key-hex> <iv-hex> <data-hex>\n", name, name); return 1; } \
        const char *op = argv[2]; \
        uint8_t key[KEY_SIZE]; size_t key_len; \
        if (hex_to_bytes(argv[3], strlen(argv[3]), key, &key_len) != CRYPTO_OK || key_len != KEY_SIZE) \
            { fprintf(stderr, "Bad key\nUsage:\n" \
    ...
    else if (strcmp(cmd, "rsa") == 0)
    {
        const char *usage = "Usage:\n"
            "  crypto rsa gen\n"
            "  crypto rsa enc <pub-hex> <plain-hex>\n"
            "  crypto rsa dec <pri-hex> <cipher-hex>\n";
    ...
    else if (strcmp(cmd, "rsasha1") == 0)
    {
        const char *usage = "Usage:\n"
            "  crypto rsasha1 sign <pri-hex> <msg-hex>\n"
            "  crypto rsasha1 verify <pub-hex> <msg-hex> <sig-hex>\n";
        if (argc < 4) { fprintf(stderr, "%s", usage); return 1; }
    ...
    else if (strcmp(cmd, "ecdsa") == 0)
    {
        const char *usage = "Usage:\n"
            "  crypto ecdsa sign <pri-hex> <msg-hex>\n"
            "  crypto ecdsa verify <pub-hex> <msg-hex> <sig-hex>\n";
        if (argc < 4) { fprintf(stderr, "%s", usage); return 1; }
        if (strcmp(argv[2], "sign") == 0)
        {
            if (argc < 5) { fprintf(stderr, "%s", usage); return 1; }
