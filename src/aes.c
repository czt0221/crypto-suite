#include "crypto.h"
#include "aes.h"
#include <string.h>

static const uint8_t S[256] = {
    ,
};

static uint8_t xtime(uint8_t a) { return (uint8_t)((a << 1) ^ ((a >> 7) * 0x1B)); }

static uint8_t gf_mul(uint8_t a, uint8_t b)
{
    uint8_t r = 0;
    for (int i = 0; i < 8; i++) { if (b & 1) r ^= a; a = xtime(a); b >>= 1; }
    return r;
}

static void sub_bytes(uint8_t s[16])
{
    for (int i = 0; i < 16; i++) s[i] = S[s[i]];
}

static void shift_rows(uint8_t s[16])
{
    uint8_t t;
    t = s[1]; s[1] = s[5]; s[5] = s[9]; s[9] = s[13]; s[13] = t;
    t = s[2]; s[2] = s[10]; s[10] = t; t = s[6]; s[6] = s[14]; s[14] = t;
    t = s[3]; s[3] = s[15]; s[15] = s[11]; s[11] = s[7]; s[7] = t;
}

static void mix_columns(uint8_t s[16])
{
    for (int c = 0; c < 4; c++)
    {
        int i = c * 4;
        uint8_t a0 = s[i], a1 = s[i+1], a2 = s[i+2], a3 = s[i+3];
        s[i]   = gf_mul(2, a0) ^ gf_mul(3, a1) ^ a2 ^ a3;
        s[i+1] = a0 ^ gf_mul(2, a1) ^ gf_mul(3, a2) ^ a3;
        s[i+2] = a0 ^ a1 ^ gf_mul(2, a2) ^ gf_mul(3, a3);
        s[i+3] = gf_mul(3, a0) ^ a1 ^ a2 ^ gf_mul(2, a3);
    }
}

static void inv_sub_bytes(uint8_t s[16])
{
    static const uint8_t invS[256] = {
        ,
    };
    for (int i = 0; i < 16; i++) s[i] = invS[s[i]];
}

static void inv_shift_rows(uint8_t s[16])
{
    uint8_t t;
    t = s[13]; s[13] = s[9]; s[9] = s[5]; s[5] = s[1]; s[1] = t;
    t = s[2]; s[2] = s[10]; s[10] = t; t = s[6]; s[6] = s[14]; s[14] = t;
    t = s[3]; s[3] = s[7]; s[7] = s[11]; s[11] = s[15]; s[15] = t;
}

static void inv_mix_columns(uint8_t s[16])
{
    for (int c = 0; c < 4; c++)
    {
        int i = c * 4;
        uint8_t a0 = s[i], a1 = s[i+1], a2 = s[i+2], a3 = s[i+3];
        s[i]   = gf_mul(14, a0) ^ gf_mul(11, a1) ^ gf_mul(13, a2) ^ gf_mul(9, a3);
        s[i+1] = gf_mul(9, a0) ^ gf_mul(14, a1) ^ gf_mul(11, a2) ^ gf_mul(13, a3);
        s[i+2] = gf_mul(13, a0) ^ gf_mul(9, a1) ^ gf_mul(14, a2) ^ gf_mul(11, a3);
        s[i+3] = gf_mul(11, a0) ^ gf_mul(13, a1) ^ gf_mul(9, a2) ^ gf_mul(14, a3);
    }
}

static uint32_t load32be(const uint8_t *p)
{
    return ((uint32_t)p[0] << 24) | ((uint32_t)p[1] << 16) | ((uint32_t)p[2] << 8) | p[3];
}

static void store32be(uint8_t *p, uint32_t v)
{
    p[0] = (uint8_t)(v >> 24); p[1] = (uint8_t)(v >> 16);
    p[2] = (uint8_t)(v >> 8); p[3] = (uint8_t)v;
}

static void xor_block(uint8_t *dst, const uint8_t *a, const uint8_t *b)
{
    for (int i = 0; i < 16; i++) dst[i] = a[i] ^ b[i];
}

static void aes_encrypt_block(const uint8_t key_sched[176], const uint8_t in[16], uint8_t out[16])
{
    uint8_t s[16];
    memcpy(s, in, 16);

    for (int i = 0; i < 16; i++) s[i] ^= key_sched[i];

    for (int r = 1; r <= 10; r++)
    {
        sub_bytes(s);
        shift_rows(s);
        if (r < 10) mix_columns(s);
        for (int i = 0; i < 16; i++) s[i] ^= key_sched[r * 16 + i];
    }

    memcpy(out, s, 16);
}

static void aes_decrypt_block(const uint8_t key_sched[176], const uint8_t in[16], uint8_t out[16])
{
    uint8_t s[16];
    memcpy(s, in, 16);

    for (int i = 0; i < 16; i++) s[i] ^= key_sched[10 * 16 + i];

    for (int r = 9; r >= 1; r--)
    {
        inv_shift_rows(s);
        inv_sub_bytes(s);
        for (int i = 0; i < 16; i++) s[i] ^= key_sched[r * 16 + i];
        if (r < 10) inv_mix_columns(s);
    }

    inv_shift_rows(s);
    inv_sub_bytes(s);
    for (int i = 0; i < 16; i++) s[i] ^= key_sched[i];

    memcpy(out, s, 16);
}

static void key_expansion(const uint8_t key[16], uint8_t sched[176])
{
    uint32_t w[44];
    static const uint8_t Rcon[10] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36 };

    for (int i = 0; i < 4; i++) w[i] = load32be(key + i * 4);

    for (int i = 4; i < 44; i++)
    {
        uint32_t t = w[i - 1];
        if (i % 4 == 0)
        {
            t = (t << 8) | (t >> 24);
            uint8_t b0 = S[(t >> 24) & 0xFF], b1 = S[(t >> 16) & 0xFF];
            uint8_t b2 = S[(t >> 8) & 0xFF], b3 = S[t & 0xFF];
            t = ((uint32_t)b0 << 24) | ((uint32_t)b1 << 16) | ((uint32_t)b2 << 8) | b3;
            t ^= (uint32_t)Rcon[i / 4 - 1] << 24;
        }
        w[i] = w[i - 4] ^ t;
    }

    for (int i = 0; i < 44; i++) store32be(sched + i * 4, w[i]);
}

static int aes_crypt(const uint8_t key[AES_KEY_SIZE], const uint8_t *in, size_t in_len, uint8_t *out, size_t *out_len, int encrypt, int mode, const uint8_t iv[AES_BLOCK_SIZE])
{
    uint8_t ks[176];
    uint8_t block[16];
    uint8_t feedback[16];

    if (in_len % 16 != 0) return CRYPTO_ERR;
    if (*out_len < in_len) return CRYPTO_ERR;

    key_expansion(key, ks);

    if (mode == 0)
    {
        for (size_t i = 0; i < in_len; i += 16)
        {
            if (encrypt) aes_encrypt_block(ks, in + i, out + i);
            else         aes_decrypt_block(ks, in + i, out + i);
        }
    }
    else
    {
        if (encrypt)
        {
            memcpy(feedback, iv, 16);
            for (size_t i = 0; i < in_len; i += 16)
            {
                xor_block(block, in + i, feedback);
                aes_encrypt_block(ks, block, out + i);
                memcpy(feedback, out + i, 16);
            }
        }
        else
        {
            memcpy(feedback, iv, 16);
            for (size_t i = 0; i < in_len; i += 16)
            {
                aes_decrypt_block(ks, in + i, block);
                xor_block(out + i, block, feedback);
                memcpy(feedback, in + i, 16);
            }
        }
    }

    *out_len = in_len;
    return CRYPTO_OK;
}

int aes_ecb_encrypt(const uint8_t key[AES_KEY_SIZE], const uint8_t *plain, size_t plain_len, uint8_t *cipher, size_t *cipher_len)
{ return aes_crypt(key, plain, plain_len, cipher, cipher_len, 1, 0, NULL); }

int aes_ecb_decrypt(const uint8_t key[AES_KEY_SIZE], const uint8_t *cipher, size_t cipher_len, uint8_t *plain, size_t *plain_len)
{ return aes_crypt(key, cipher, cipher_len, plain, plain_len, 0, 0, NULL); }

int aes_cbc_encrypt(const uint8_t key[AES_KEY_SIZE], const uint8_t iv[AES_BLOCK_SIZE], const uint8_t *plain, size_t plain_len, uint8_t *cipher, size_t *cipher_len)
{ return aes_crypt(key, plain, plain_len, cipher, cipher_len, 1, 1, iv); }

int aes_cbc_decrypt(const uint8_t key[AES_KEY_SIZE], const uint8_t iv[AES_BLOCK_SIZE], const uint8_t *cipher, size_t cipher_len, uint8_t *plain, size_t *plain_len)
{ return aes_crypt(key, cipher, cipher_len, plain, plain_len, 0, 1, iv); }
